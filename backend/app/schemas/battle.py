...
class UseSkillRequest(BaseModel):
    ...
    turn_no: int
    actor_combatant_id: int
    target_combatant_id: int | None = Field(
        default=None,
        description="single_enemy / single_ally では必須。self は API 側で actor を自動採用する。",
    )
    skill_key: str
    declared_tactic_text: str = ""
    risk_level: str = "LOW"
...
class UsableSkillItem(BaseModel):
    skill_key: str
    name: str
    category: str
    target_type: str = Field(description="single_enemy / single_ally / self")
    resource_type: str
    resource_cost: int
    cooldown_turns: int
...
class UsableSpellItem(BaseModel):
    spell_key: str
    name: str
    category: str
    target_type: str = Field(description="single_enemy / single_ally / self")
...
class CombatantUsablesResponse(BaseModel):
    battle_id: int
    combatant_id: int
    skill_items: list[UsableSkillItem]
    spell_items: list[UsableSpellItem]
